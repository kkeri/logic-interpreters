...
reduceDual initial terminal = reduce where
  reduce e p = rewriteRec "reduce" reduce' e p where
    ...
    -- variables
    reduce' (Def n v) (Name o)  | n == o = opCase "subst" v   -- variable resolution
    reduce' a (Def n v)         = do w <- reduce a v; return (Def n w) -- var. backprop
...
parse ::
  forall env tm val.
  (String -> tm) -> -- name constructor
  (String -> tm -> tm) -> -- let constructor
  Ip env tm val -> -- interpret terms between {...}
  Ip env tm val -> -- interpret terms between [...]
  [Token] ->
  IO (tm, [Token])
parse consName consDef bracesIp bracketsIp = parseTerm
  where
    parseTerm :: [Token] -> IO (tm, [Token])
    parseTerm input = case input of
      []                 -> error "unexpected end of input"
      TName "let" : rest -> parseDef rest
      TName str : rest   -> return (consName str, rest)
      TDelim '{' : rest  ->
    ...
    parseDef :: [Token] -> IO (tm, [Token])
    parseDef input = case input of
      []                 -> error "unexpected end of input"
      TName str : rest   -> do
                              (val, rest') <- parseTerm rest
                              return (consDef str val, rest')
...
parseTm = parse Name Def conjIp disjIp

-- Consume input without computation.
skipTerm = parse (const ()) (const $ const ()) voidIp voidIp
...
printTm (Def n t) = do putStr "let "; putStr n; putChar ' '; printTm t
...
printBin (Def n t) = do putStr "(let "; putStr n; putChar ' '; printTm t; putChar ')'
